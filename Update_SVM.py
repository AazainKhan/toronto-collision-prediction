...
# Load the dataset
df = pd.read_csv(r"C:\Users\YounghunMun\Desktop\SupervisedLearning_274\project\df_group5.csv")
...
# Splitting the data
X_train, X_test, y_train, y_test = train_test_split(X, y_encoded, test_size=0.3, random_state=5, stratify=y_encoded)
...
random_search = RandomizedSearchCV(pipeline, param_distributions=param_distributions, n_iter=10, cv=3, verbose=2, random_state=5, scoring='accuracy')
...
plt.plot(fpr_after, tpr_after, color='darkorange', lw=2, label='ROC curve (area = %0.2f)' % auc_roc_after)
