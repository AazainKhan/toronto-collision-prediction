...
df_official.to_csv(r"C:\Users\YounghunMun\Desktop\SupervisedLearning_274\project\cleaned_data_KSI.csv", index=False)
...
svm_model = SVC(kernel='rbf', probability=True, random_state=5)
svm_model.fit(x_train_prepared, y_train_encoded)  # Use encoded labels here

# Predict on the training set and test set using the trained model
y_train_pred = svm_model.predict(x_train_prepared)
y_test_pred = svm_model.predict(full_pipeline.transform(x_test))
...
# Generating and plotting ROC curve for the test set predictions
y_test_probabilities = svm_model.predict_proba(full_pipeline.transform(x_test))[:, 1]
fpr, tpr, _ = roc_curve(y_test_encoded, y_test_probabilities)  # Use encoded labels here
roc_auc = auc(fpr, tpr)
